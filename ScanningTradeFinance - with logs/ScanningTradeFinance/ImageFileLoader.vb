...
Public Class ImageFileLoader
    ...
    Public Function Load(ByVal owner As Window, ByVal codecs As RasterCodecs, ByVal autoLoad As Boolean) As Boolean
        ...
        If (ofd.ShowDialog(owner) = DialogResult.OK) Then
            ...
            If (info.Format = RasterImageFormat.RasPdf OrElse
               info.Format = RasterImageFormat.RasPdfG31Dim OrElse
               info.Format = RasterImageFormat.RasPdfG32Dim OrElse
               info.Format = RasterImageFormat.RasPdfG4 OrElse
               info.Format = RasterImageFormat.RasPdfJpeg OrElse
               info.Format = RasterImageFormat.RasPdfJpeg422 OrElse
               info.Format = RasterImageFormat.RasPdfJpeg411) Then
                If (Not codecs.Options.Pdf.IsEngineInstalled) Then
                    'Dim dlg As New PdfEngi
                ...
                Case RasterDialogFileOptionsType.Misc
                    Select Case (firstItem.FileInfo.Format)
                        Case RasterImageFormat.Jbig
                            'Set the user options
                            codecs.Options.Jbig.Load.Resolution = New LeadSize(
                               firstItem.Options.MiscOptions.XResolution,
                               firstItem.Options.MiscOptions.YResolution)
                        Case RasterImageFormat.Cmw
                            'Set the user options
                            codecs.Options.Jpeg2000.Load.CmwResolution = New LeadSize(
                               firstItem.Options.MiscOptions.XResolution,
                               firstItem.Options.MiscOptions.YResolution)

                        Case RasterImageFormat.Jp2
                            'Set the user options
                            codecs.Options.Jpeg2000.Load.Jp2Resolution = New LeadSize(
                               firstItem.Options.MiscOptions.XResolution,
                               firstItem.Options.MiscOptions.YResolution)

                        Case RasterImageFormat.J2k
                            'Set the user options
                            codecs.Options.Jpeg2000.Load.J2kResolution = New LeadSize(
                               firstItem.Options.MiscOptions.XResolution,
                               firstItem.Options.MiscOptions.YResolution)
                    End Select
        ...
        Return ok
    End Function
    Public Function Loadscanimage(ByVal owner As Window, ByVal codecs As RasterCodecs, ByVal autoLoad As Boolean, ByVal filename As String, ByVal totpage As Integer) As Boolean
        _image = codecs.Load(filename, 0, CodecsLoadByteOrder.BgrOrGray, 1, totpage)

        Return True
    End Function
    Public Function Load(ByVal owner As Window, ByVal codecs As RasterCodecs, ByVal autoLoad As Boolean, ByVal fpage As Integer, ByVal lpage As Integer, ByVal filename As String) As Boolean
        Dim ofd As New RasterOpenDialog(codecs)
        Dim ok As Boolean = False
        ok = True
