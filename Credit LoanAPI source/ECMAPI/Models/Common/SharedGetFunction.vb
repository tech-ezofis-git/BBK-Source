...
#End Region
    ...
    Public Shared Function IsValidDate(dateStr As String) As Boolean
        ' Define the regex pattern for dd-mmm-yyyy format
        Dim pattern As String = "^(0[1-9]|[12][0-9]|3[01])-(0[1-9]|1[0-2])-(\d{4})$"
        Dim regex As New Regex(pattern, RegexOptions.IgnoreCase)
        Dim match As Match = regex.Match(dateStr)
        Return match.Success
    End Function
    Public Shared Function CREDITUploadFn(para As InsUpload, EcmLoginId As Integer, CallHistoryId As Integer) As resmessage
        Dim resmsg As New resmessage()
        Dim logf As String = "Entered CreditUploadfn"
        Try
            Dim strqry = "", filename = "", xmlfilename = "", filepath = "", xmlfilepath = "", strRimNumber = ""
            ...
            Dim strloanType = "", strSubLoanType = "", strDocumentType = ""
            ...
            If para.CabinetName <> "" Then
                ...
                If Not IsNothing(dsTemplate) AndAlso dsTemplate.Tables.Count > 0 AndAlso dsTemplate.Tables(0).Rows.Count > 0 Then
                    ...
                    If templateId > 0 Then
                        Dim templateList = SelectedeZTemplateList("TemplateId", templateId)
                        If Not IsNothing(templateList) AndAlso templateList.Count > 0 Then
                            strqryHistory = "update eZCREDITAPICallHistory set CabinetId=" + templateList(0).CabinetID.ToString + " ,TemplateId=" + templateList(0).TemplateId.ToString + " ,Createdby='" + EcmLoginId.ToString + "',UpdatedBy='" + EcmLoginId.ToString + "' where CallHistoryId='" + CallHistoryId.ToString + "'"
                            UCallHistoryId = InsertAndUpdateAndDeleteeZUserDefinedWithScope(strqryHistory)
                            logf &= " eZCREDITAPICallHistory updated cabinet "
                            If Not IsNothing(para.Fields) AndAlso para.Fields.Count > 0 Then
                                ...
                                For i As Integer = 0 To fieldsList.Count - 1
                                    ...
                                    For Each inputFieldList In para.Fields
                                        If fieldsList(i).FieldName.ToLower = inputFieldList.FieldName.ToLower Then
                                            ...
                                            If fieldsList(i).FieldName.ToLower = "rim number" Then
                                                strRimNumber = inputFieldList.FieldValue
                                            ElseIf fieldsList(i).FieldName.Replace(" ", "").ToLower() = "loantype" Then
                                                strloanType = inputFieldList.FieldValue
                                            ElseIf fieldsList(i).FieldName.Replace(" ", "").ToLower() = "subloantype" Then
                                                strSubLoanType = inputFieldList.FieldValue
                                            ElseIf fieldsList(i).FieldName.Replace(" ", "").ToLower() = "documenttype" Then
                                                strDocumentType = inputFieldList.FieldValue
                                            End If
                                            'If docFileNameField.FieldName = "" Then
                                            '    If fieldsList(i).FieldName.Replace(" ", "").ToLower() = "documenttype" Then
                                    ...
                                    If hasfield = False Then
                                        resmsg.errorCode = 5
                                        resmsg.value = "Error code: 3_2 - Input Fieldname(" + fieldsList(i).FieldName + ") not found"
                                        strqryHistory = "update eZCREDITAPICallHistory set Remarks='" + resmsg.value + "',[RIM Number]='" + strRimNumber + "', UpdatedOnAPI='" + DateDateTimeToString(Date.Now, True) + "' where CallHistoryId='" + CallHistoryId.ToString + "'"
                                        UCallHistoryId = InsertAndUpdateAndDeleteeZUserDefinedWithScope(strqryHistory)
                                        Return resmsg
                                    End If
                                Next
                                ...
                                If (ds_ErsID IsNot Nothing And ds_ErsID.Tables.Count > 0 And ds_ErsID.Tables(0).Rows.Count > 0) Then
                                    ...
                                    If para.filetype <> "" Then
                                        ...
                                        Dim extension As String = "." & para.filetype
                                        ...
                                        If fileExtensions.Contains(extension.ToLower()) Then
                                            ...
                                            filename = strDocumentType + "." + para.filetype.ToLower()
                                        Else
                                            If extension <> "" Then
                                        ...
                                        filepath = strloanType + "\" + strSubLoanType + "\" + strRimNumber + "\"
                                        ...
                                        If System.IO.File.Exists(System.IO.Path.Combine(ersvalue.ERSDirPath, templateList(0).CabinetName, templateList(0).TemplateName, filepath, filename.Replace("." + para.filetype, ".ezo"))) Then
                                            ...
                                            orgFileName = orgFileName.Replace(Path.GetExtension(orgFileName), "") & "_" & version & Path.GetExtension(orgFileName)
                                        ...
                                        Try
                                            ...
                                            End Using
                                        Catch ex As Exception
                                            logf &= " Failed to write file in " & archievePath & ": " & ex.Message.ToString()
                                        End Try
                                        ...
                                        Dim orgfilepath As String = System.IO.Path.Combine(ersvalue.ERSDirPath, templateList(0).CabinetName, templateList(0).TemplateName, filepath, orgFileName)
                                        'File.WriteAllBytes(orgfilepath, buffer)
                                        Try
                                            Using fs As New FileStream(orgfilepath, FileMode.Create, FileAccess.Write, FileShare.None)   ' ⬅ exclusive lock
                                                fs.Write(buffer, 0, buffer.Length)
                                                fs.Flush(True)
                                            End Using
                                        Catch ex As Exception
                                            logf &= " Failed to write file in " & orgfilepath & ": " & ex.Message.ToString()
                                        End Try
                                        encodeType = "1"
                                        strqry = " insert into ezca_" + templateList(0).CabinetID.ToString() + "_" + templateId.ToString() + "_items"
                                        strqry += " (ERSId,TemplateId," + strqrycols + "ifilepath,ifilename,ifiletype,ezFrom,Version,dstatus,dsize,createdon,createdBy,encodeType) values(" & ersId & "," & templateId.ToString() & "," & strqryvals & "" & "'" + templateList(0).CabinetName + "\" + templateList(0).TemplateName + "\" + filepath.Replace("'", "''") + "'" & " , '" & filename.Replace("'", "''") & "','" & para.filetype.ToLower() & "','CREDITAPI(Client-PC)','" & version & "','Active','0','" & DateDateTimeToString(Date.Now, True) & "','" + EcmLoginId.ToString + "'," + encodeType + ")"
                                        logf &= "qry " & strqry


                                        ItemId = InsertAndUpdateAndDeleteeZUserDefinedWithScope(strqry)
                                        Dim res = New With {.ItemId = ItemId, .URL = APIurl + "/V1/Credit/viewfile/" & para.CabinetName & "/" & ItemId}
                                        ...
                                        strqryHistory = "update eZCREDITAPICallHistory set ItemId = '" + ItemId + "',Status = 'Archived', Remarks='" + resmsg.value + "',[RIM Number]='" + strRimNumber + "', UpdatedOnAPI='" + DateDateTimeToString(Date.Now, True) + "',encodeType=" + encodeType + " where CallHistoryId='" + CallHistoryId.ToString + "'"
                                    ...
                                    Else
                                        ...
                                        strqryHistory = "update eZCREDITAPICallHistory set Remarks='" + resmsg.value + "',[RIM Number]='" + strRimNumber + "',encodeType=" + encodeType + " , UpdatedOnAPI='" + DateDateTimeToString(Date.Now, True) + "' where CallHistoryId='" + CallHistoryId.ToString + "'"
                                        UCallHistoryId = InsertAndUpdateAndDeleteeZUserDefined(strqryHistory)
                                    End If
                                End If
                            Else
        ...
        Catch ex As Exception
            Dim strqryHistory = "update eZCREDITAPICallHistory set Remarks='Exception: " + ex.ToString + "' where CallHistoryId='" + CallHistoryId.ToString + "'"
            Dim UCallHistoryId = InsertAndUpdateAndDeleteeZUserDefinedWithScope(strqryHistory)
            resmsg.errorCode = 2
