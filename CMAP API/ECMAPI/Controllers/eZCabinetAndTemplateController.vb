...
Namespace Controllers
    Public Class eZCabinetAndTemplateController
        ...
        Function GetItemsByCriteria(ByVal Para As SearchRegistries) As DataSet
            ...
            End If
            ItemListCount = GetDatasetByQuery("Select isnull(SUM(nopages),0) from " + Tablename + " where isdeleted=0  " + CondtionReg)
            Dim PageCount As String = "0"
            If Not IsNothing(ItemListCount) AndAlso ItemListCount.Tables.Count > 0 AndAlso ItemListCount.Tables(0).Rows.Count > 0 Then
                PageCount = ItemListCount.Tables(0).Rows(0)(0).ToString()
            End If
            ItemListCount = GetDatasetByQuery("Select isnull(SUM(nopages),0) from " + Tablename + " where isdeleted=0 and ezfrom like 'Scanned%' " + CondtionReg)
            Dim PageCount_Scanned As String = "0"
            If Not IsNothing(ItemListCount) AndAlso ItemListCount.Tables.Count > 0 AndAlso ItemListCount.Tables(0).Rows.Count > 0 Then
                PageCount_Scanned = ItemListCount.Tables(0).Rows(0)(0).ToString()
            End If
            ItemListCount = GetDatasetByQuery("Select isnull(SUM(nopages),0) from " + Tablename + " where isdeleted=0 and ezfrom like 'Digital%' " + CondtionReg)
     ...
     End Function
            ...
            Dim LogTestQuery As Action(Of String, Integer) =
    Sub(queryText As String, resultCount As Integer)
        Try
            InsertQueryListAudit(queryText, resultCount, "SUCCESS", "", Tablename, CondtionReg)
        Catch ex As Exception
            InsertQueryListAudit(queryText, 0, "ERROR", ex.ToString(), Tablename, CondtionReg)
        End Try
    End Sub
            ...
            'itemlistcountTotal
            testQuery = "Select Count(1) from " + Tablename + " where isdeleted=0 " + CondtionReg
            LogTestQuery(testQuery, ToInt(TotalRow))

            'itemlistcountTotal_scanned
            testQuery = "Select Count(1) from " + Tablename + " where isdeleted=0 and ezfrom like 'Scanned%' " + CondtionReg
            LogTestQuery(testQuery, ToInt(TotalRow_Scanned))

            'itemlistcountTotal_digital
            testQuery = "Select Count(1) from " + Tablename + " where isdeleted=0 and ezfrom like 'Digital%' " + CondtionReg
            LogTestQuery(testQuery, ToInt(TotalRow_Digital))

            'itemlistpagecount
            testQuery = "Select isnull(SUM(nopages),0) from " + Tablename + " where isdeleted=0 " + CondtionReg
            LogTestQuery(testQuery, ToInt(PageCount))

            'itemlistpagecount-scanned
            testQuery = "Select isnull(SUM(nopages),0) from " + Tablename + " where isdeleted=0 and ezfrom like 'Scanned%' " + CondtionReg
            LogTestQuery(testQuery, ToInt(PageCount_Scanned))

            'itemlistpagecount-Digital
            testQuery = "Select isnull(SUM(nopages),0) from " + Tablename + " where isdeleted=0 and ezfrom like 'Digital%' " + CondtionReg
            LogTestQuery(testQuery, ToInt(PageCount_Digital))

            'itemlistrimcount
            testQuery = "Select Count(1) from (select distinct [RIM NUMBER] from " + Tablename + " where isdeleted=0  " + CondtionReg + ") as x"
            LogTestQuery(testQuery, ToInt(RIMCount))

            'itemlistrimcount-scanned
            testQuery = "Select Count(1) from (select distinct [RIM NUMBER] from " + Tablename + " where isdeleted=0 and ezfrom like 'Scanned%'  " + CondtionReg + ") as x"
            LogTestQuery(testQuery, ToInt(RIMCount_Scanned))

            'itemlistrimcount-digital
            testQuery = "Select Count(1) from (select distinct [RIM NUMBER] from " + Tablename + " where isdeleted=0 and ezfrom like 'Digital%'  " + CondtionReg + ") as x"
            LogTestQuery(testQuery, ToInt(RIMCount_Digital))

            'itemlistfinalquery
            LogTestQuery(FinalQuery, 0)

            '============================Testing ends=================

            Dim ItemList As DataSet = Nothing
            Try
                ItemList = GetDatasetByQuery(FinalQuery)
                Dim returnCount As Integer = 0
                If Not IsNothing(ItemList) AndAlso ItemList.Tables.Count > 0 Then
                    returnCount = ItemList.Tables(0).Rows.Count
                End If
                InsertQueryListAudit(FinalQuery, returnCount, "SUCCESS", "", Tablename, CondtionReg)
            Catch ex As Exception
                InsertQueryListAudit(FinalQuery, 0, "ERROR", ex.ToString(), Tablename, CondtionReg)
                Throw
            End Try
            ...
            '  Return ReurnResults
        End Function
        Private Sub InsertQueryListAudit(queryText As String, resultCount As Integer, status As String, errorText As String, tableName As String, conditionText As String)
            Try
                Dim escQuery As String = If(queryText, "").Replace("'", "''")
                Dim escStatus As String = If(status, "").Replace("'", "''")
                Dim escError As String = If(errorText, "").Replace("'", "''")
                Dim escTable As String = If(tableName, "").Replace("'", "''")
                Dim escCondition As String = If(conditionText, "").Replace("'", "''")
                Dim escCreatedOn As String = DateDateTimeToString(Date.Now, True).Replace("'", "''")

                Dim logQry As String =
            "IF OBJECT_ID('dbo.querylist','U') IS NOT NULL BEGIN " &
            "INSERT INTO dbo.querylist " &
            "(querytext, resultcount, returndatacount, status, errormessage, [error], tablename, conditiontext, createdon) VALUES (" &
            "N'" & escQuery & "', " &
            ...
            "); END"
                ...
                InsertAndUpdate(logQry)
            Catch
                ' Do not interrupt API flow if logging table/columns differ by environment.
            End Try
        End Sub
        ...
        Function GetItemsFieldSearch(ByVal Para As SearchRegistries) As DataSet()
            ...
            If Flag = 0 Then
                ...
                If Not IsNothing(ItemListCount) AndAlso ItemListCount.Tables.Count > 0 AndAlso ItemListCount.Tables(0).Rows.Count > 0 Then
                    TotalRow = ItemListCount.Tables(0).Rows(0)(0).ToString()
                End If
                ItemListCount = GetDatasetByQuery("Select isnull(SUM(nopages),0) from " + Tablename + " where isdeleted=0 and ifilename <> '' " + CondtionReg)
            ...
            ElseIf Flag = 1 Then
                ...
                If Not IsNothing(ItemListCount) AndAlso ItemListCount.Tables.Count > 0 AndAlso ItemListCount.Tables(0).Rows.Count > 0 Then
                    TotalRow = ItemListCount.Tables(0).Rows(0)(0).ToString()
                End If
                ItemListCount = GetDatasetByQuery("Select isnull(SUM(nopages),0) from (Select [Rim Number] from " + Tablename + " where isdeleted=0 and ifilename <> '' " + CondtionReg + "   group by [RIM Number],CheckOut,convert(date,createdon,106),CreatedBy) as x")
                Dim PageCount As String = ""
                If Not IsNothing(ItemListCount) AndAlso ItemListCount.Tables.Count > 0 AndAlso ItemListCount.Tables(0).Rows.Count > 0 Then
                    PageCount = ItemListCount.Tables(0).Rows(0)(0).ToString()
                End If
            ...
            ElseIf Flag = 5 Then
                ...
                Else
                    If Para.RowCount <> 0 Then
                        '   FinalQuery = "Select *  ,dbo.udf_LoginName (CreatedBy) as LoginName ,'" + PageCount + "' as PageCount, '" + TotalRow + "' as TotalRow from " + Tablename + " where isdeleted=0  " + CondtionReg & " and Seq BETWEEN " + RowFrom.ToString() + " AND " + RowCount.ToString()
                        FinalQuery = "SELECT * FROM (Select [RIM Number],case when CheckOut='' then 'CheckIn' else checkout end as [Status],Count(1) as [File Count],isnull(sum(nopages),0) as [Page Count],(select top 1 substring(createdon,0,12) from eZCA_3_9_items where [RIM Number]=itm.[RIM Number] order by itemid) as [Created Date],dbo.udf_LoginName((select top 1 createdby from eZCA_3_9_items where [RIM Number]=itm.[RIM Number] order by itemid)) as [Created User] ,'" + PageCount + "' as PageCount, '" + TotalRow + "' as TotalRow , ROW_NUMBER() OVER (ORDER BY itemid) AS Seq from " + Tablename + " where isdeleted=0 and ifilename <> '' " + CondtionReg & ")t" + " WHERE Seq BETWEEN " + Para.RowFrom.ToString() + " AND " + Para.RowCount.ToString() + " group by [RIM Number],CheckOut,convert(date,createdon,106),CreatedBy"
                        ' strQry = "SELECT Transactionid FROM (" + strQry.Replace("select Transactionid", "select Transactionid, ROW_NUMBER() OVER (ORDER BY Transactionid) AS Seq") + ")t" + " WHERE Seq BETWEEN " + RowFrom.ToString() + " AND " + RowCount.ToString()
                    Else
                        FinalQuery = "Select [RIM Number],case when CheckOut='' then 'CheckIn' else checkout end as [Status],Count(1) as [File Count],isnull(sum(nopages),0) as [Page Count],(select top 1 substring(createdon,0,12) from eZCA_3_9_items where [RIM Number]=itm.[RIM Number] order by itemid)) as [Created Date],dbo.udf_LoginName((select top 1 createdby from eZCA_3_9_items where [RIM Number]=itm.[RIM Number] order by itemid)) as [Created User] ,'" + PageCount + "' as PageCount, '" + TotalRow + "' as TotalRow  from " + Tablename + " where isdeleted=0 and ifilename <> ''  " + CondtionReg + +" group by [RIM Number],CheckOut,convert(date,createdon,106),CreatedBy"
                    End If
            ...
            ElseIf Flag = 2 Then
                ...
                If Not IsNothing(ItemListCount) AndAlso ItemListCount.Tables.Count > 0 AndAlso ItemListCount.Tables(0).Rows.Count > 0 Then
                    TotalRow = ItemListCount.Tables(0).Rows(0)(0).ToString()
                End If
                ItemListCount = GetDatasetByQuery("select isnull(sum(itm.nopages),0) from ezusersession us left join eZCA_3_9_items itm on us.itemid=itm.itemid where us.TemplateId='9' and  us.TemplateId=9 and ifilename<>''")
                Dim PageCount As String = ""
                If Not IsNothing(ItemListCount) AndAlso ItemListCount.Tables.Count > 0 AndAlso ItemListCount.Tables(0).Rows.Count > 0 Then
                    PageCount = ItemListCount.Tables(0).Rows(0)(0).ToString()
                End If
            ...
            ElseIf Flag = 3 Then
                ...
                If Not IsNothing(ItemListCount) AndAlso ItemListCount.Tables.Count > 0 AndAlso ItemListCount.Tables(0).Rows.Count > 0 Then
                    TotalRow = ItemListCount.Tables(0).Rows(0)(0).ToString()
                End If
                ItemListCount = GetDatasetByQuery("select isnull(sum(itm.nopages),0)  from ezindexingchange ic left join ezca_3_9_items itm on itm.TemplateId=ic.Templateid and itm.itemid=ic.itemid where itm.isdeleted=0 and itm.ifilename<>'' and Indexingchangeid in (select (select top 1 Indexingchangeid from eZIndexingChange where itemid=ic.itemid order by Indexingchangeid desc) from eZIndexingChange ic group by Templateid,FieldId,itemid)")
                Dim PageCount As String = ""
                If Not IsNothing(ItemListCount) AndAlso ItemListCount.Tables.Count > 0 AndAlso ItemListCount.Tables(0).Rows.Count > 0 Then
                    PageCount = ItemListCount.Tables(0).Rows(0)(0).ToString()
                End If
        ...
        Function GetItemsFieldSearchReport(ByVal Para As SearchRegistries) As DataSet
            ...
            ElseIf Flag = 5 Then
                ...
                If Not IsNothing(ItemListCount) AndAlso ItemListCount.Tables.Count > 0 AndAlso ItemListCount.Tables(0).Rows.Count > 0 Then
                    TotalRow = ItemListCount.Tables(0).Rows(0)(0).ToString()
                End If
                ItemListCount = GetDatasetByQuery("Select isnull(SUM(nopages),0) from (Select [Rim Number] from " + Tablename + " where isdeleted=0 and ifilename <> '' " + CondtionReg + "   group by [RIM Number],CheckOut) as x")
                Dim PageCount As String = ""
                If Not IsNothing(ItemListCount) AndAlso ItemListCount.Tables.Count > 0 AndAlso ItemListCount.Tables(0).Rows.Count > 0 Then
                    PageCount = ItemListCount.Tables(0).Rows(0)(0).ToString()
                End If
                If ConfigurationManager.AppSettings("SqlAbouve2012") = "true" Then
                    If Para.RowCount <> 0 Then
                        FinalQuery = "Select [RIM Number],case when CheckOut='' then 'CheckIn' else checkout end as [Status],Count(1) as [File Count],isnull(sum(nopages),0) as [Page Count],(select top 1 convert(date,substring(createdon,0,12),106) from eZCA_3_9_items where [RIM Number]=itm.[RIM Number] order by itemid) as [Created Date],dbo.udf_LoginName((select top 1 createdby from eZCA_3_9_items where [RIM Number]=itm.[RIM Number] order by itemid)) as [Created User] ,'" + PageCount + "' as PageCount, '" + TotalRow + "' as TotalRow from " + Tablename + " itm where isdeleted=0 and ifilename <> '' " + CondtionReg & " group by [RIM Number],CheckOut order by [Created Date]  OFFSET " + Para.RowFrom.ToString() + " ROWS FETCH NEXT " + Para.RowCount.ToString() + " ROWS ONLY"
                    Else
                        FinalQuery = "Select [RIM Number],case when CheckOut='' then 'CheckIn' else checkout end as [Status],Count(1) as [File Count],isnull(sum(nopages),0) as [Page Count],(select top 1 convert(date,substring(createdon,0,12),106) from eZCA_3_9_items where [RIM Number]=itm.[RIM Number] order by itemid) as [Created Date],dbo.udf_LoginName((select top 1 createdby from eZCA_3_9_items where [RIM Number]=itm.[RIM Number] order by itemid)) as [Created User],'" + PageCount + "' as PageCount, '" + TotalRow + "' as TotalRow from " + Tablename + " itm where isdeleted=0 and ifilename <> ''  " + CondtionReg & " group by [RIM Number],CheckOut order by [Created Date] "
                    End If
                Else
            ...
            ElseIf Flag = 3 Then
                ...
                If Not IsNothing(ItemListCount) AndAlso ItemListCount.Tables.Count > 0 AndAlso ItemListCount.Tables(0).Rows.Count > 0 Then
                    TotalRow = ItemListCount.Tables(0).Rows(0)(0).ToString()
                End If
                ItemListCount = GetDatasetByQuery("select isnull(sum(itm.nopages),0)  from ezindexingchange ic left join ezca_3_9_items itm on itm.TemplateId=ic.Templateid and itm.itemid=ic.itemid where itm.isdeleted=0 and itm.ifilename<>'' " + CondtionReg & " and Indexingchangeid in (select (select top 1 Indexingchangeid from eZIndexingChange where itemid=ic.itemid and FieldId=59 order by Indexingchangeid desc) from eZIndexingChange ic group by Templateid,FieldId,itemid)")
                Dim PageCount As String = ""
                If Not IsNothing(ItemListCount) AndAlso ItemListCount.Tables.Count > 0 AndAlso ItemListCount.Tables(0).Rows.Count > 0 Then
                    PageCount = ItemListCount.Tables(0).Rows(0)(0).ToString()
                End If
